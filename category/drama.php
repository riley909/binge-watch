...
// 장르 아이디로 리스트를 불러온다
  ...
  data: { "api_key": "d579af00349a9e85a6a32ff41c93ad8c", "page": "1", "with_genres": "18" },
  dataType: "json"
})
  .done(function( dramaResult ) {
    for (i = 0; i < dramaResult.results.length; i++) {
      var image = dramaResult.results[i].poster_path == null ? "/menu/img/no-image-icon-23485.png" : "https://image.tmdb.org/t/p/w185/" + dramaResult.results[i].poster_path;
      var name = dramaResult.results[i].name;
      var firstAirDate = dramaResult.results[i].first_air_date;
      var tv_id = dramaResult.results[i].id;
